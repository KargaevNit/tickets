...
const getOccupiedPlaces = async () => {
  ...
  console.log(occupiedPlacesDictionary.value.includes("1_1"));
};
...
const isSelected = (place: Place): boolean|number => {
  let res = false;
  selectedPlace.value.forEach((sel_place: Place, idx: number) => {
...
const totalPrice = computed(() => {
  let total = 0;
  selectedPlace.value.forEach((place: Place) => {
    total += place.price;
  });
  return total;
});
...
<template>
  ...
  <Modal title="Оплата" :component="OrderModal" />
</template>
